...
inline void sharedBoundaryTet(
  HexIndex *common, unsigned int &nbCommon,
  const HXTCombineCell& cell0, const HXTCombineCell& cell1)
{
  nbCommon = 0;
  int i0 = 0, i1 = 0;
  while (i0!=cell0.numBoundaryTets && i1!=cell1.numBoundaryTets) {
    unsigned int t0 = cell0.boundaryTetrahedra[i0];
    unsigned int t1 = cell1.boundaryTetrahedra[i1];
    if (t0 < t1) ++i0;
    else if (t1 < t0) ++i1;
    else{
      common[nbCommon++] = cell0.tet(t0);
      break;
    }
  }
}

inline bool sharedFacet(
  HexIndex *common, unsigned int &nbCommon,
  const HXTCombineCell& cell0, const HXTCombineCell& cell1)
{
  nbCommon = 0;
  ...
  sharedVertices(commonVertices, nbCommonVertices, cell0, cell1);
  if(nbCommonVertices == 4){
   quadindex facet(commonVertices[0], commonVertices[1], commonVertices[2], commonVertices[3]);
   if (!isQuadFacet(cell0, facet)) return false;
   else if (!isQuadFacet(cell1, facet)) return false;
   else return true;
  }else return false;
}
