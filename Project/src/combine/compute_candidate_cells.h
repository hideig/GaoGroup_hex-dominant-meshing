...
inline bool isCellReallyValid(const TetMeshWrapper& tets, const vector<VertexIndex>& vertices)
{
  ...
  if      (vertices.size() == 8) return hexValidity(points.data());
  else if (vertices.size() == 6) return prismValidity(points.data());
  else if (vertices.size() == 5) return pyramidValidity(points.data()); 
  else return false;
}
